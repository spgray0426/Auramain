 ...
 * Aura 프로젝트의 어빌리티 시스템 유틸리티 라이브러리 구현
 *
 * Blueprint Function Library로 어디서든 호출 가능한 정적 헬퍼 함수들을 제공합니다:
 * - 위젯 컨트롤러 접근
 * - 캐릭터 속성/어빌리티 초기화
 * - 데미지 이펙트 적용
 * - 이펙트 컨텍스트 Getter/Setter
 * - 타겟 탐지 및 유틸리티 함수
 */
...
/**
 * 위젯 컨트롤러 파라미터 생성 헬퍼
 * 플레이어 컨트롤러에서 필요한 컴포넌트들을 가져와 FWidgetControllerParams 구조체를 채웁니다.
 */
...
/**
 * 캐릭터 기본 속성 초기화
 * CharacterClassInfo 데이터 에셋에서 클래스별 기본 속성을 가져와 적용합니다.
 * Primary -> Secondary -> Vital 순서로 GameplayEffect를 적용합니다.
 */
...
/**
 * 세이브 데이터로부터 캐릭터 속성 초기화
 * 저장된 게임을 로드할 때 이전 속성값들을 복원합니다.
 * SetByCaller를 사용하여 저장된 정확한 값을 적용합니다.
 */
...
/**
 * 클래스와 레벨에 따른 XP 보상 계산
 * 적 처치 시 획득하는 경험치를 계산합니다.
 */
...
/** 게임모드에서 CharacterClassInfo 데이터 에셋 반환 */
...
/*
 * ===========================================
 * EffectContext Getter 함수들
 * FAuraGameplayEffectContext에서 커스텀 데이터를 읽어옵니다.
 * ===========================================
 */
...
/*
 * ===========================================
 * EffectContext Setter 함수들
 * FAuraGameplayEffectContext에 커스텀 데이터를 설정합니다.
 * ===========================================
 */
...
/**
 * 가장 가까운 타겟들 선택
 * 주어진 액터 목록에서 원점에서 가장 가까운 MaxTargets 개의 타겟을 반환합니다.
 */
...
/**
 * 두 액터가 적대 관계인지 확인
 * Player 태그와 Enemy 태그를 비교하여 같은 팀인지 판단합니다.
 */
...
/**
 * 데미지 이펙트 적용
 * DamageEffectParams 구조체의 모든 파라미터를 사용하여
 * 데미지 GameplayEffect를 생성하고 타겟에 적용합니다.
 */
...
/**
 * 균등 간격 회전값 배열 생성
 * 다중 발사체 스킬에서 균등하게 퍼지는 방향을 계산합니다.
 */
...
/**
 * 균등 간격 방향 벡터 배열 생성
 * 다중 발사체 스킬에서 균등하게 퍼지는 방향 벡터를 계산합니다.
 */
